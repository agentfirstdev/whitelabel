...
Webpages can be localized with <Link href='whitelabel/geotargeting'>geotargeting parameters</Link>.
They're retrieved in real time by default or optionally queued for <Link
href='whitelabel/scheduling'>subsequent retrieval</Link>. Up to **120 seconds** is allotted per
real-time API call to accommodate CAPTCHA-solving, multiple retries, and other mitigations.
...
| `session`    | ⬜       | Any unique identifier of up to 255 characters (regardless of character encoding); **{companyName}** will make best efforts to route calls in the same session to the same egress node                                                               |
