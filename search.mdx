...
Besides the [geotargeting and scheduling params](#service-usage) linked above, required and optional
...
| `awaiting`   | ⬜       | The lazy result type to await loading, `ai` overview or "People also ask" `answers`; an AI overview can take significant time to generate and is awaited for up to **1 minute**; multiple types can be provided by repeating the key; lazy types aren't awaited by default                                                                                                                                       |
