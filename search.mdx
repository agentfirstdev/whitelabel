...
| `callback`   | ⬜       | The encoded HTTP or HTTPS callback URL or Amazon SQS queue URL or ARN to notify when the (prerequisite) asynchronous results have been retrieved; any SQS queue must allow the AWS principal `arn:aws:iam::180363035301:role/api-instance` to perform `sqs:SendMessage` actions                                                                                                                                  |
...
params to **https://&zwj;{apiEndpoint}/search/devices**:
