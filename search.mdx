...
iconType: light
description: Extract popular real-time search results.
---
...
**{companyName}'s** search service delivers all organic and paid results reliably, as structured or
raw data. The service has a RESTful interface that accepts `GET` requests at
...
in which case up to **2 minutes** is allocated per API call. <Link href='reporting'>Usage
data</Link> is published hourly.
