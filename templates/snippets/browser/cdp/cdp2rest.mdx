...
const submissionUrl = 'https://httpbin.org/post';
const sizeSelector = '[type="radio"][value="medium"]';
const toppingSelector = '[type="checkbox"][value="onion"]';
...
let response = await page.goto(formUrl);

if (response?.ok()) {
  console.log('Title:', await page.title());
} else {
  console.error(
    'API interaction failure:',
    await page.evaluate(() => {
...
try {
  response = await fetch(
    `${restEndpoint}?url=${encodeURIComponent(submissionUrl)}&expiration=0`,
